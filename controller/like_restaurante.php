...
// ID del usuario (obtenido de la sesión y sanitizado) para verificar los restaurantes a los que le dio like
$id_documento = isset($_SESSION['user_tour']['id_documento']) ? (int)$_SESSION['user_tour']['id_documento'] : 0;
$id_restaurante = isset($_POST['id_restaurante']) ? filter_var($_POST['id_restaurante'], FILTER_SANITIZE_NUMBER_INT) : null;
...
if ($id_restaurante && $id_documento) {
    // Llama a la función toggleLike
    $resultado = toggleLike($conexion, $id_restaurante, $id_documento, $like_status);
